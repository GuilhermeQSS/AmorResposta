...
-- Tabela de catálogo de materiais (estoque)
-- Obs: se a tabela 'materiais' já existir com a estrutura antiga (enc_id, item_id),
-- ela precisará ser migrada manualmente. Esta instrução cria a nova tabela somente
-- se ainda não houver nenhuma tabela com esse nome.
SET @create_materiais = IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.TABLES
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'materiais') = 0,
  'CREATE TABLE materiais (
    mat_id INT AUTO_INCREMENT PRIMARY KEY,
    mat_nome VARCHAR(255) NOT NULL,
    mat_quantidade_total INT NOT NULL DEFAULT 0
  )',
  'SELECT 1'
);
PREPARE stmt FROM @create_materiais;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Tabela de relação entre encontros e materiais
CREATE TABLE IF NOT EXISTS `encontro_materiais` (
  `enc_id` INT NOT NULL,
  `mat_id` INT NOT NULL,
  `enc_mat_quantidade_usada` INT NOT NULL,
  PRIMARY KEY (`enc_id`, `mat_id`),
  CONSTRAINT `fk_enc_mat_encontros`
    FOREIGN KEY (`enc_id`) REFERENCES `encontros` (`enc_id`) ON DELETE CASCADE,
  CONSTRAINT `fk_enc_mat_materiais`
    FOREIGN KEY (`mat_id`) REFERENCES `materiais` (`mat_id`) ON DELETE CASCADE
);

-- Coluna fun_id em encontros (responsável pelo encontro)
SET @add_enc_fun_id = IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'encontros'
      AND COLUMN_NAME = 'fun_id') = 0,
  'ALTER TABLE encontros ADD COLUMN fun_id INT NOT NULL DEFAULT 0',
  'SELECT 1'
);
PREPARE stmt FROM @add_enc_fun_id;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- Coluna enc_status em encontros
SET @add_enc_status = IF(
  (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    WHERE TABLE_SCHEMA = DATABASE()
      AND TABLE_NAME = 'encontros'
      AND COLUMN_NAME = 'enc_status') = 0,
  "ALTER TABLE encontros ADD COLUMN enc_status VARCHAR(50) NOT NULL DEFAULT 'Planejado'",
  'SELECT 1'
);
PREPARE stmt FROM @add_enc_status;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- FK de encontros.fun_id para funcionarios
CALL `add_foreign_key_if_missing`(
  'fk_encontros_funcionarios',
  'ALTER TABLE `encontros` ADD CONSTRAINT `fk_encontros_funcionarios` FOREIGN KEY (`fun_id`) REFERENCES `funcionarios` (`fun_id`)'
);
